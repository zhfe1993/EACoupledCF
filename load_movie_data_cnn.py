...
def load_rating_file_as_list():
    ratingList = []
    with open('ml-1m/ml-1m.test.rating',"r",encoding='UTF-8') as f:
        line = f.readline()
...
def load_negative_file():
    negativeList = []
    with open('ml-1m/ml-1m.test.negative',"r",encoding='UTF-8') as f:
        line = f.readline()
        while line != None and line != "":
            arr = line.split("\t")
            negatives = []
...
def load_rating_train_as_matrix():
    ...
    with open("ml-1m/u.info","r",encoding='UTF-8') as f:
        line = f.readline()
        while line != None and line != "":
            arr = line.split(" ")
            if(arr[1].replace("\n", "") == 'users'):
    ...
    with open('ml-1m/ml-1m.train.rating',"r",encoding='UTF-8') as f:
        line = f.readline()
        while line != None and line != "":
            arr = line.split("\t")
...
def load_itemGenres_as_matrix():
    ...
    with open("ml-1m/u.genre","r",encoding='UTF-8') as f:
        line = f.readline().strip('\n')
        while line != None and line != "":
            arr = line.split("|")
            dict[arr[0]] = num_type
    ...
    with open("ml-1m/movies.dat","r",encoding='UTF-8') as f:
        line = f.readline().strip('\r\n')
        while line != None and line != "":
            arr = line.split("::")
            types = arr[2].split("|")
...
def load_user_attributes():
    ...
    with open('ml-1m/users.dat',"r",encoding='UTF-8') as f:
        line = f.readline().strip('\n')
        while line != None and line != "":
            arr = line.split('::')
            l = []
